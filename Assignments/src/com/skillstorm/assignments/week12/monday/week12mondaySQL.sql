...
-- SELECT rental_id, customer_id, return_date FROM sakila.rental;

# 5. Get the film titles and descriptions of films that have a rating of "R".
-- SELECT title, description FROM sakila.film WHERE rating = "R";

# 6. Retrieve the rental ID, customer ID, and return date of rentals that are overdue (return date is in the past).
-- SELECT rental_id, customer_id, return_date FROM sakila.rental WHERE return_date < NOW();
# OR...
-- SELECT rental_id, customer_id, return_date FROM sakila.rental WHERE return_date < last_update;
...
# starts with "S".
# I didn't see a phone number column for customers, so I used the email column instead.
-- SELECT first_name, last_name, email FROM sakila.customer WHERE last_name LIKE "S%";